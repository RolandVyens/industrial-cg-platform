...
- **Deep EXR direct scene-output merge parity fix (2026-03-25):** the remaining direct-file regression on `light-passes-test-v001.blend` was traced to a split between the callback write path and the RenderResult handoff path. `finalize_deep_output()` could already write merged deep samples, but `get_processed_deep_data()` still returned raw `processed_cache_`. The current fix shares one final merge helper across both paths, restoring direct scene-output parity with the compositor-style final merge without re-enabling the older early generic Cycles merge. CPU + OptiX Nuke previews (`C:\tmp\deep_merge_cpu_preview_20260325.png`, `C:\tmp\deep_merge_optix_preview_20260325.png`) are currently visually clean, while the broader hard-surface compaction checker remains a separate follow-up task.
...
### Deep EXR early merge rollback (2026-03-24)
- Evidence: the current direct Deep EXR output (`D:\blender_projects\rendered\test\trash_output\.exr`)
  now deviates from the known-good artifacts (`C:\tmp\direct_scene_output_write1.png`,
  `C:\tmp\scene_output_rgba_deep_0002.exr`, `C:\tmp\scene_output_rgba_deep_saved_0001.exr`), with
  sample counts collapsed to ~38.2M from the historical ~69.5M / ~74.7M and
  `check_deep_volume_passthrough.py` reporting ~444,038 mismatched volume pixels. These symptoms
  align with the early generic `merge_nearby_samples()` pass merging volume segments before the
  later per-surface work runs.
- Decision: abandon the generic early merge/compaction roll-forward for all Deep EXR output paths.
  Disable that pass, preserve volume structure, take the correctness hit first, and accept larger
  deep sample counts/files instead of merging segments prematurely. Documented design note:
  `docs/plans/2026-03-24-deep-exr-early-merge-disable-design.md`.
...
- Deep EXR hard-surface opaque-coverage follow-up (2026-03-23): the remaining DeepMerge seam after
  ...
  Nuke artifacts in `C:\tmp\direct_scene_output_saved_*.png` show the large white seam
...
- Validation-tooling correction (2026-03-23): the current deep rerun on
  `C:\tmp\scene_output_rgba_deep_probe_####.exr` confirmed the visual DeepMerge seam is back in the
  acceptable fixed state (`C:\tmp\direct_scene_output_saved_write1.png`). During that rerun,
  several `.agent` checkers were fixed to use native deep data/channel indices correctly. In
  particular, the previous mixed surface/volume "failure" was found to be a bad checker rather than
  a confirmed renderer regression; after fixing the script, `.agent/check_deep_mixed_surface_volume_case1.py`
  returns `mismatching_pixels=0` on the current probe file.
...
## Deep EXR Status Update - 2026-03-25

- Active local fix direction is now **metadata-gated safe-case reconstruction** in
  `intern/cycles/session/deep_output_driver.cpp`.
- Only the proven safe **surface-front-prefix + volume-behind** case is allowed to use the new
  hard-surface correction path.
- Pure volume, volume-front mixed, and ambiguous mixed pixels stay on the legacy fallback path.
- Validation on `D:\blender_projects\light-passes-test-v001.blend` with the runtime RGBA deep
  override now reports:
  - `flatten_matches_flat=1`
  - `pixels_gt_0.05=13`
  - no large validated volume-hole regression in the flattened result

## Deep EXR Matrix Recheck - 2026-03-25

- The locked matrix in `.agent/DEEP_EXR_TEST_MATRIX.md` was re-run using fresh matched files in
  `C:\tmp\` for CPU and OptiX.
- Current result is **not fully green yet**.
- OptiX refreshed direct/compositor-RGBA/compositor-alpha-only files currently pass the required
  script gate that is available in this environment.
- CPU refreshed RGBA-deep variants still fail `check_deep_flatten_matches_flat.py` even though the
  obvious seam/volume-hole regression is not the dominant visible issue anymore.
- Current blocker is now treated as a **broad CPU flatten-vs-flat RGB mismatch**, strongest in
