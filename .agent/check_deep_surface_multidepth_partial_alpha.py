...
def flatten_deep_alpha(
    deep_data, width: int, x: int, y: int, alpha_channel: int
) -> tuple[float, int]:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Check known multi-depth partial hard-surface pixels where flattened deep alpha "
            "must match the flat alpha."
        )
    )
    ...
    print(f"checked_multidepth_partial_pixels={len(KNOWN_PIXELS)}")
    print(f"mismatching_multidepth_partial_pixels={mismatching}")
