...
static string environment_string_get(const char *name)
{
...
static OptixCachePlatform optix_cache_platform_get()
{
#  ifdef _WIN32
  return OptixCachePlatform::Windows;
#  elif defined(__APPLE__)
  return OptixCachePlatform::MacOS;
#  else
  return OptixCachePlatform::Linux;
#  endif
}
      ...
      launch_params(this, "kernel_params", false)
{
  ...
  const OptixCacheEnvironment cache_environment = {environment_string_get("OPTIX_CACHE_PATH"),
                                                   environment_string_get("LOCALAPPDATA"),
                                                   environment_string_get("HOME"),
                                                   environment_string_get("XDG_CACHE_HOME")};
  const OptixCachePolicy cache_policy = optix_cache_policy_resolve(optix_cache_platform_get(),
                                                                   cache_environment);
  if (cache_policy.use_product_path) {
    const string &cache_path = cache_policy.path;
